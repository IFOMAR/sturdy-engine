...
;
var C = (a, t, e) => (be(a, t, "access private method"),
e);
function H() {
    return {
        async: !1,
...
function Te(a) {
    return a.replace($e, (t, e) => (e = e.toLowerCase(),
    e === "colon" ? ":" : e.charAt(0) === "#" ? e.charAt(1) === "x" ? String.fromCharCode(parseInt(e.substring(2), 16)) : String.fromCharCode(+e.substring(1)) : ""))
}
const ze = /(^|[^\[])\^/g;
function k(a, t) {
...
`, 1)[0]
                  , f = 0;
                this.options.pedantic ? (f = 2,
                o = u.trimStart()) : (f = e[2].search(/[^ ]/),
                f = f > 4 ? 1 : f,
                o = u.slice(f),
                f += e[1].length);
                let y = !1;
...
`,
                ...
                !c) {
                    const M = new RegExp(`^ {0,${Math.min(3, f - 1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`)
                      ...
                      , V = new RegExp(`^ {0,${Math.min(3, f - 1)}}(?:\`\`\`|~~~)`)
                      , Y = new RegExp(`^ {0,${Math.min(3, f - 1)}}#`);
                    for (; t; ) {
                        const D = t.split(`
`, 1)[0];
                        if (h = D,
                        this.options.pedantic && (h = h.replace(/^ {1,4}(?=( {4})*[^ ])/g, "  ")),
                        V.test(h) || Y.test(h) || M.test(h) || K.test(t))
                            break;
                        if (h.search(/[^ ]/) >= f || !h.trim())
                            o += `
` + h.slice(f);
                        else {
                            if (y || u.search(/[^ ]/) >= 4 || V.test(u) || Y.test(u) || K.test(u))
                                break;
                            o += `
` + h
                        }
    ...
    def(t) {
        ...
        if (e) {
            const n = e[1].toLowerCase().replace(/\s+/g, " ")
              , i = e[2] ? e[2].replace(/^<(.*)>$/, "$1").replace(this.rules.inline.anyPunctuation, "$1") : ""
...
` ? e[1].slice(0, -1) : e[1];
    ...
    link(t) {
        ...
        if (e) {
            ...
            if (this.options.pedantic) {
                ...
                r = e[3] ? e[3].slice(1, -1) : "";
            return i = i.trim(),
            /^</.test(i) && (this.options.pedantic && !/>$/.test(n) ? i = i.slice(1) : i = i.slice(1, -1)),
            se(e, {
    ...
    emStrong(t, e, n="") {
        ...
        if (!(i[1] || i[2] || "") || !n || this.rules.inline.punctuation.exec(n)) {
            ...
            for (u.lastIndex = 0,
            e = e.slice(-1 * t.length + s); (i = u.exec(e)) != null; ) {
                if (l = i[1] || i[2] || i[3] || i[4] || i[5] || i[6],
                !l)
                    continue;
                if (o = [...l].length,
                i[3] || i[4]) {
...
const Se = /^(?: *(?:\n|$))+/
  ...
  , X = /<!--(?:-?>|[\s\S]*?(?:-->|$))/
  , Ze = k("^ {0,3}(?:<(script|pre|style|textarea)[\\s>][\\s\\S]*?(?:</\\1>[^\\n]*\\n+|$)|comment[^\\n]*(\\n+|$)|<\\?[\\s\\S]*?(?:\\?>\\n*|$)|<![A-Z][\\s\\S]*?(?:>\\n*|$)|<!\\[CDATA\\[[\\s\\S]*?(?:\\]\\]>\\n*|$)|</?(tag)(?: +|\\n|/?>)[\\s\\S]*?(?:(?:\\n *)+\\n|$)|<(?!script|pre|style|textarea)([a-z][\\w-]*)(?:attribute)*? */?>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n *)+\\n|$)|</(?!script|pre|style|textarea)[a-z][\\w-]*\\s*>(?=[ \\t]*(?:\\n|$))[\\s\\S]*?(?:(?:\\n *)+\\n|$))", "i").replace("comment", X).replace("tag", Q).replace("attribute", / +[a-zA-Z:_][\w.:-]*(?: *= *"[^"\n]*"| *= *'[^'\n]*'| *= *[^\s"'=<>`]+)?/).getRegex()
  , he = k(F).replace("hr", A).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("|table", "").replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", Q).getRegex()
  ...
  , Be = {
    ...G,
    table: ie,
    paragraph: k(F).replace("hr", A).replace("heading", " {0,3}#{1,6}(?:\\s|$)").replace("|lheading", "").replace("table", ie).replace("blockquote", " {0,3}>").replace("fences", " {0,3}(?:`{3,}(?=[^`\\n]*\\n)|~{3,})[^\\n]*\\n").replace("list", " {0,3}(?:[*+-]|1[.)]) ").replace("html", "</?(?:tag)(?: +|\\n|/?>)|<(?:script|pre|style|textarea|!--)").replace("tag", Q).getRegex()
}
  , ve = {
    ...G,
  ...
  , Me = /^(`+|[^`])(?:(?= {2,}\n)|[\s\S]*?(?:(?=[\\<!\[`*_]|\b_|$)|[^ ](?= {2,}\n)))/
  , E = "\\p{P}\\p{S}"
  , De = k(/^((?![*_])[\spunctuation])/, "u").replace(/punctuation/g, E).getRegex()
  , Oe = /\[[^[\]]*?\]\([^\(\)]*?\)|`[^`]*?`|<[^<>]*?>/g
  , je = k(/^(?:\*+(?:((?!\*)[punct])|[^\s*]))|^_+(?:((?!_)[punct])|([^\s_]))/, "u").replace(/punct/g, E).getRegex()
  , Ne = k("^[^_*]*?__[^_*]*?\\*[^_*]*?(?=__)|[^*]+(?=[^*])|(?!\\*)[punct](\\*+)(?=[\\s]|$)|[^punct\\s](\\*+)(?!\\*)(?=[punct\\s]|$)|(?!\\*)[punct\\s](\\*+)(?=[^punct\\s])|[\\s](\\*+)(?!\\*)(?=[punct])|(?!\\*)[punct](\\*+)(?!\\*)(?=[punct])|[^punct\\s](\\*+)(?=[^punct\\s])", "gu").replace(/punct/g, E).getRegex()
  , He = k("^[^_*]*?\\*\\*[^_*]*?_[^_*]*?(?=\\*\\*)|[^_]+(?=[^_])|(?!_)[punct](_+)(?=[\\s]|$)|[^punct\\s](_+)(?!_)(?=[punct\\s]|$)|(?!_)[punct\\s](_+)(?=[^punct\\s])|[\\s](_+)(?!_)(?=[punct])|(?!_)[punct](_+)(?!_)(?=[punct])", "gu").replace(/punct/g, E).getRegex()
  , Fe = k(/\\([punct])/, "gu").replace(/punct/g, E).getRegex()
  , Ue = k(/^<(scheme:[^\s\x00-\x1f<>]*|email)>/).replace("scheme", /[a-zA-Z][a-zA-Z0-9+.-]{1,31}/).replace("email", /[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+(@)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)+(?![-_])/).getRegex()
  , Xe = k(X).replace("(?:-->|$)", "-->").getRegex()
  , Ge = k("^comment|^</[a-zA-Z][\\w:-]*\\s*>|^<[a-zA-Z][\\w-]*(?:attribute)*?\\s*/?>|^<\\?[\\s\\S]*?\\?>|^<![a-zA-Z]+\\s[\\s\\S]*?>|^<!\\[CDATA\\[[\\s\\S]*?\\]\\]>").replace("comment", Xe).replace("attribute", /\s+[a-zA-Z:_][\w.:-]*(?:\s*=\s*"[^"]*"|\s*=\s*'[^']*'|\s*=\s*[^\s"'=<>`]+)?/).getRegex()
  , P = /(?:\[(?:\\.|[^\[\]\\])*\]|\\.|`[^`]*`|[^\[\]\\`])*?/
  , We = k(/^!?\[(label)\]\(\s*(href)(?:\s+(title))?\s*\)/).replace("label", P).replace("href", /<(?:\\.|[^\n<>\\])+>|[^\s\x00-\x1f]*/).replace("title", /"(?:\\"?|[^"\\])*"|'(?:\\'?|[^'\\])*'|\((?:\\\)?|[^)\\])*\)/).getRegex()
  , fe = k(/^!?\[(label)\]\[(ref)\]/).replace("label", P).replace("ref", U).getRegex()
  , ge = k(/^!?\[(ref)\](?:\[\])?/).replace("ref", U).getRegex()
  ...
  , Ke = {
    ...W,
    link: k(/^!?\[(label)\]\((.*?)\)/).replace("label", P).getRegex(),
    reflink: k(/^!?\[(label)\]\s*\[([^\]]*)\]/).replace("label", P).getRegex()
}
  , j = {
    ...W,
...
class m {
    constructor(t) {
        ...
        this.options.pedantic ? (e.block = q.pedantic,
        e.inline = R.pedantic) : this.options.gfm && (e.block = q.gfm,
        this.options.breaks ? e.inline = R.breaks : e.inline = R.gfm),
...
`),
    ...
    blockTokens(t, e=[]) {
        ...
        let n, i, r, s;
        for (; t; )
            if (!(this.options.extensions && this.options.extensions.block && this.options.extensions.block.some(l => (n = l.call({
                lexer: this
            ...
            !0) : !1))) {
                if (n = this.tokenizer.space(t)) {
                    ...
                    n.raw.length === 1 && e.length > 0 ? e[e.length - 1].raw += `
` : e.push(n);
                    continue
                }
...
` + n.raw,
                    i.text += `
` + n.raw,
                    this.inlineQueue[this.inlineQueue.length - 1].src = i.text) : this.tokens.links[n.tag] || (this.tokens.links[n.tag] = {
                        href: n.href,
                        title: n.title
                    });
                    continue
...
` + n.text,
    ...
    inlineTokens(t, e=[]) {
        ...
        for (; t; )
            if (o || (p = ""),
            o = !1,
            !(this.options.extensions && this.options.extensions.inline && this.options.extensions.inline.some(c => (n = c.call({
                lexer: this
            }, t, e)) ? (t = t.substring(n.raw.length),
            e.push(n),
            !0) : !1))) {
                ...
                if (n = this.tokenizer.tag(t)) {
                    t = t.substring(n.raw.length),
                    i = e[e.length - 1],
                    i && n.type === "text" && i.type === "text" ? (i.raw += n.raw,
            ...
            case "list":
                {
                    ...
                    for (let u = 0; u < s.items.length; u++) {
                        ...
                        if (h.task) {
                            ...
                            p ? h.tokens.length > 0 && h.tokens[0].type === "paragraph" ? (h.tokens[0].text = $ + " " + h.tokens[0].text,
                            h.tokens[0].tokens && h.tokens[0].tokens.length > 0 && h.tokens[0].tokens[0].type === "text" && (h.tokens[0].tokens[0].text = $ + " " + h.tokens[0].tokens[0].text)) : h.tokens.unshift({
                                type: "text",
                                text: $ + " "
                            }) : b += $ + " "
                        }
                        b += this.parse(h.tokens, p),
                        c += this.renderer.listitem(b, y, !!f)
                    }
...
` + (s.tokens ? this.parseInline(s.tokens) : s.text);
    ...
    parseInline(t, e) {
        ...
        for (let i = 0; i < t.length; i++) {
            const r = t[i];
            if (this.options.extensions && this.options.extensions.renderers && this.options.extensions.renderers[r.type]) {
                const s = this.options.extensions.renderers[r.type].call({
                    parser: this
                }, r);
...
d(S, "passThroughHooks", new Set(["preprocess", "postprocess", "processAllTokens"]));
var I, N, v, ke;
class Ye {
    constructor(...t) {
        O(this, I);
        ...
        d(this, "parse", C(this, I, N).call(this, m.lex, w.parse));
        d(this, "parseInline", C(this, I, N).call(this, m.lexInline, w.parseInline));
        d(this, "Parser", w);
        d(this, "Renderer", B);
    ...
    walkTokens(t, e) {
        ...
        let n = [];
        for (const s of t)
            switch (n = n.concat(e.call(this, s)),
            s.type) {
            case "table":
                {
                    const l = s;
    ...
    use(...t) {
        ...
        return t.forEach(n => {
            ...
            n.extensions && (n.extensions.forEach(r => {
                if (!r.name)
                ...
                if ("tokenizer"in r) {
                    ...
                    r.start && (r.level === "block" ? e.startBlock ? e.startBlock.push(r.start) : e.startBlock = [r.start] : r.level === "inline" && (e.startInline ? e.startInline.push(r.start) : e.startInline = [r.start]))
                }
...
N = function(t, e) {
    return (n, i) => {
        ...
        this.defaults.async === !0 && r.async === !1 && (s.silent || console.warn("marked(): The async option was set to true by an extension. The async: false option sent to parse will be ignored."),
        s.async = !0);
        const l = C(this, v, ke).call(this, !!s.silent, !!s.async);
        ...
        try {
            ...
            let p = e(o, s);
            return s.hooks && (p = s.hooks.postprocess(p)),
            p
        } catch (o) {
            return l(o)
        }
...
g.options = g.setOptions = function(a) {
    return T.setOptions(a),
    g.defaults = T.defaults,
    re(g.defaults),
    g
}
;
