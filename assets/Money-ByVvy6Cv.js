import {S as k, i as w, s as z, Q as C, R as H, T as u, m as A, U as v, c, b as S, V as B, W as U, X as W, Y as q, t as D, d as P, h as m, Z as b, _ as Q, $ as R, D as T, p as Z} from "./index-BsTNpDop.js";
function X(r) {
    let e = (console.error('Unsupported icon weight. Choose from "thin", "light", "regular", "bold", "fill", or "duotone".'),
    ""), t;
    return {
        c() {
            t = T(e)
        },
        m(l, i) {
            c(l, t, i)
        },
        p: Z,
        d(l) {
            l && m(t)
        }
    }
}
function Y(r) {
    let e;
    return {
        c() {
            e = u("path"),
            A(e, "d", "M128,164a36,36,0,1,0-36-36A36,36,0,0,0,128,164Zm0-64a28,28,0,1,1-28,28A28,28,0,0,1,128,100ZM240,60H16a4,4,0,0,0-4,4V192a4,4,0,0,0,4,4H240a4,4,0,0,0,4-4V64A4,4,0,0,0,240,60ZM20,107.26A52.75,52.75,0,0,0,59.26,68H196.74A52.75,52.75,0,0,0,236,107.26v41.48A52.75,52.75,0,0,0,196.74,188H59.26A52.75,52.75,0,0,0,20,148.74ZM236,99a44.8,44.8,0,0,1-31-31h31ZM51,68A44.8,44.8,0,0,1,20,99V68ZM20,157a44.8,44.8,0,0,1,31,31H20Zm185,31a44.8,44.8,0,0,1,31-31v31Z")
        },
        m(t, l) {
            c(t, e, l)
        },
        p: Z,
        d(t) {
            t && m(e)
        }
    }
}
function j(r) {
    let e;
    return {
        c() {
            e = u("path"),
            A(e, "d", "M128,88a40,40,0,1,0,40,40A40,40,0,0,0,128,88Zm0,64a24,24,0,1,1,24-24A24,24,0,0,1,128,152ZM240,56H16a8,8,0,0,0-8,8V192a8,8,0,0,0,8,8H240a8,8,0,0,0,8-8V64A8,8,0,0,0,240,56ZM193.65,184H62.35A56.78,56.78,0,0,0,24,145.65v-35.3A56.78,56.78,0,0,0,62.35,72h131.3A56.78,56.78,0,0,0,232,110.35v35.3A56.78,56.78,0,0,0,193.65,184ZM232,93.37A40.81,40.81,0,0,1,210.63,72H232ZM45.37,72A40.81,40.81,0,0,1,24,93.37V72ZM24,162.63A40.81,40.81,0,0,1,45.37,184H24ZM210.63,184A40.81,40.81,0,0,1,232,162.63V184Z")
        },
        m(t, l) {
            c(t, e, l)
        },
        p: Z,
        d(t) {
            t && m(e)
        }
    }
}
function E(r) {
    let e;
    return {
        c() {
            e = u("path"),
            A(e, "d", "M128,166a38,38,0,1,0-38-38A38,38,0,0,0,128,166Zm0-64a26,26,0,1,1-26,26A26,26,0,0,1,128,102ZM240,58H16a6,6,0,0,0-6,6V192a6,6,0,0,0,6,6H240a6,6,0,0,0,6-6V64A6,6,0,0,0,240,58ZM22,108.82A54.73,54.73,0,0,0,60.82,70H195.18A54.73,54.73,0,0,0,234,108.82v38.36A54.73,54.73,0,0,0,195.18,186H60.82A54.73,54.73,0,0,0,22,147.18ZM234,96.29A42.8,42.8,0,0,1,207.71,70H234ZM48.29,70A42.8,42.8,0,0,1,22,96.29V70ZM22,159.71A42.8,42.8,0,0,1,48.29,186H22ZM207.71,186A42.8,42.8,0,0,1,234,159.71V186Z")
        },
        m(t, l) {
            c(t, e, l)
        },
        p: Z,
        d(t) {
            t && m(e)
        }
    }
}
function F(r) {
    let e;
    return {
        c() {
            e = u("path"),
            A(e, "d", "M168,128a40,40,0,1,1-40-40A40,40,0,0,1,168,128Zm80-64V192a8,8,0,0,1-8,8H16a8,8,0,0,1-8-8V64a8,8,0,0,1,8-8H240A8,8,0,0,1,248,64Zm-16,46.35A56.78,56.78,0,0,1,193.65,72H62.35A56.78,56.78,0,0,1,24,110.35v35.3A56.78,56.78,0,0,1,62.35,184h131.3A56.78,56.78,0,0,1,232,145.65Z")
        },
        m(t, l) {
            c(t, e, l)
        },
        p: Z,
        d(t) {
            t && m(e)
        }
    }
}
function G(r) {
    let e, t;
    return {
        c() {
            e = u("path"),
            t = u("path"),
            A(e, "d", "M160,128a32,32,0,1,1-32-32A32,32,0,0,1,160,128Zm40-64a48.85,48.85,0,0,0,40,40V64Zm0,128h40V152A48.85,48.85,0,0,0,200,192ZM16,152v40H56A48.85,48.85,0,0,0,16,152Zm0-48A48.85,48.85,0,0,0,56,64H16Z"),
            A(e, "opacity", "0.2"),
            A(t, "d", "M128,88a40,40,0,1,0,40,40A40,40,0,0,0,128,88Zm0,64a24,24,0,1,1,24-24A24,24,0,0,1,128,152ZM240,56H16a8,8,0,0,0-8,8V192a8,8,0,0,0,8,8H240a8,8,0,0,0,8-8V64A8,8,0,0,0,240,56ZM24,72H45.37A40.81,40.81,0,0,1,24,93.37Zm0,112V162.63A40.81,40.81,0,0,1,45.37,184Zm208,0H210.63A40.81,40.81,0,0,1,232,162.63Zm0-38.35A56.78,56.78,0,0,0,193.65,184H62.35A56.78,56.78,0,0,0,24,145.65v-35.3A56.78,56.78,0,0,0,62.35,72h131.3A56.78,56.78,0,0,0,232,110.35Zm0-52.28A40.81,40.81,0,0,1,210.63,72H232Z")
        },
        m(l, i) {
            c(l, e, i),
            c(l, t, i)
        },
        p: Z,
        d(l) {
            l && (m(e),
            m(t))
        }
    }
}
function I(r) {
    let e;
    return {
        c() {
            e = u("path"),
            A(e, "d", "M240,52H16A12,12,0,0,0,4,64V192a12,12,0,0,0,12,12H240a12,12,0,0,0,12-12V64A12,12,0,0,0,240,52ZM181.21,180H74.79A60.18,60.18,0,0,0,28,133.21V122.79A60.18,60.18,0,0,0,74.79,76H181.21A60.18,60.18,0,0,0,228,122.79v10.42A60.18,60.18,0,0,0,181.21,180ZM228,97.94A36.23,36.23,0,0,1,206.06,76H228ZM49.94,76A36.23,36.23,0,0,1,28,97.94V76ZM28,158.06A36.23,36.23,0,0,1,49.94,180H28ZM206.06,180A36.23,36.23,0,0,1,228,158.06V180ZM128,88a40,40,0,1,0,40,40A40,40,0,0,0,128,88Zm0,56a16,16,0,1,1,16-16A16,16,0,0,1,128,144Z")
        },
        m(t, l) {
            c(t, e, l)
        },
        p: Z,
        d(t) {
            t && m(e)
        }
    }
}
function J(r) {
    let e, t, l, i;
    const d = r[7].default
      , o = C(d, r, r[6], null);
    function g(a, n) {
        return a[0] === "bold" ? I : a[0] === "duotone" ? G : a[0] === "fill" ? F : a[0] === "light" ? E : a[0] === "regular" ? j : a[0] === "thin" ? Y : X
    }
    let _ = g(r)
      , s = _(r)
      , M = [{
        xmlns: "http://www.w3.org/2000/svg"
    }, {
        width: r[2]
    }, {
        height: r[2]
    }, {
        fill: r[1]
    }, {
        transform: l = r[3] ? "scale(-1, 1)" : void 0
    }, {
        viewBox: "0 0 256 256"
    }, r[4], r[5]]
      , h = {};
    for (let a = 0; a < M.length; a += 1)
        h = H(h, M[a]);
    return {
        c() {
            e = u("svg"),
            o && o.c(),
            t = u("rect"),
            s.c(),
            A(t, "width", "256"),
            A(t, "height", "256"),
            A(t, "fill", "none"),
            v(e, h)
        },
        m(a, n) {
            c(a, e, n),
            o && o.m(e, null),
            S(e, t),
            s.m(e, null),
            i = !0
        },
        p(a, [n]) {
            o && o.p && (!i || n & 64) && B(o, d, a, a[6], i ? W(d, a[6], n, null) : U(a[6]), null),
            _ === (_ = g(a)) && s ? s.p(a, n) : (s.d(1),
            s = _(a),
            s && (s.c(),
            s.m(e, null))),
            v(e, h = q(M, [{
                xmlns: "http://www.w3.org/2000/svg"
            }, (!i || n & 4) && {
                width: a[2]
            }, (!i || n & 4) && {
                height: a[2]
            }, (!i || n & 2) && {
                fill: a[1]
            }, (!i || n & 8 && l !== (l = a[3] ? "scale(-1, 1)" : void 0)) && {
                transform: l
            }, {
                viewBox: "0 0 256 256"
            }, a[4], n & 32 && a[5]]))
        },
        i(a) {
            i || (D(o, a),
            i = !0)
        },
        o(a) {
            P(o, a),
            i = !1
        },
        d(a) {
            a && m(e),
            o && o.d(a),
            s.d()
        }
    }
}
function K(r, e, t) {
    const l = ["weight", "color", "size", "mirrored"];
    let i = b(e, l)
      , {$$slots: d={}, $$scope: o} = e;
    const {weight: g, color: _, size: s, mirrored: M, ...h} = Q("iconCtx") || {};
    let {weight: a=g ?? "regular"} = e
      , {color: n=_ ?? "currentColor"} = e
      , {size: V=s ?? "1em"} = e
      , {mirrored: p=M || !1} = e;
    return r.$$set = f => {
        e = H(H({}, e), R(f)),
        t(5, i = b(e, l)),
        "weight"in f && t(0, a = f.weight),
        "color"in f && t(1, n = f.color),
        "size"in f && t(2, V = f.size),
        "mirrored"in f && t(3, p = f.mirrored),
        "$$scope"in f && t(6, o = f.$$scope)
    }
    ,
    [a, n, V, p, h, i, o, d]
}
class N extends k {
    constructor(e) {
        super(),
        w(this, e, K, J, z, {
            weight: 0,
            color: 1,
            size: 2,
            mirrored: 3
        })
    }
}
export {N as M};
